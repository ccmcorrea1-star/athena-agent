...
Athena saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.athena/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
athena -c                  # Continue most recent session
athena -r                  # Browse and select from past sessions
athena --no-session        # Ephemeral mode; do not save
athena --name "my task"    # Set session display name at startup
athena --session <path|id> # Use a specific session file or partial session ID
athena --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `athena -r` opens the same picker at startup.
...
When available, athena uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
athena --name "Refactor auth module"
athena --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `athena -r`.
...
When `/tree` switches away from one branch to another, athena can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
