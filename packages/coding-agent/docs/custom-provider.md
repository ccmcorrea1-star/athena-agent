...
Extensions can register custom model providers via `athena.registerProvider()`. This enables:
...
Extensions can register either a complete athena-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Athena composes `models.json` overrides above registered native providers.
...
export default function (athena: ExtensionAPI) {
  athena.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  athena.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. athena waits for the factory before startup continues, so the provider is available during interactive startup and to `athena --list-models`.
...
athena.registerProvider("openai", {
  headers: {
...
athena.registerProvider("google", {
...
```typescript
athena.registerProvider("my-llm", {
...
Use `athena.unregisterProvider(name)` to remove a provider that was previously registered via `athena.registerProvider(name, ...)`:
...
// Later, remove it
athena.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map athena levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
athena.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@athena/ai";
...
Credentials are persisted in `~/.athena/agent/auth.json`:
...
- [anthropic.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, athena can recover automatically by compacting the conversation and retrying. This recovery only kicks in if athena recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of athena's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/athena-agent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message athena does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase athena recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (athena: ExtensionAPI) {
  ...
  athena.on("message_end", (event, ctx) => {
...
`message_end` runs before athena tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what athena checks. With this in place, athena will:
...
- Match a provider-specific pattern, not athena's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of athena's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/earendil-works/athena-agent/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps athena thinking levels to provider/model-specific values; null marks a level unsupported. */
