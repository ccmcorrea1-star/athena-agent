...
```bash
curl -fsSL https://athena.dev/install.sh | sh
```

To uninstall athena itself, use npm for curl and npm installs:

```bash
npm uninstall -g @athena/coding-agent
```

For pnpm, Yarn, or Bun installs, use the matching global remove command: `pnpm remove -g @athena/coding-agent`, `yarn global remove @athena/coding-agent`, or `bun uninstall -g @athena/coding-agent`.
