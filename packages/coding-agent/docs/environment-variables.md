...
- Athena sets `ATHENA_CODING_AGENT` so child processes can detect that they run inside Athena.
...
The CLI and RPC entry points set `ATHENA_CODING_AGENT=true`. Child processes inherit it and can use it to detect that they run inside Athena. It is not session-specific and is not set automatically when Athena is embedded through the SDK.
...
| `ATHENA_SKIP_VERSION_CHECK` | Disable the `athena.dev` latest-version request |
