> athena can create themes. Ask it to build one for your setup.
...
Athena loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.athena/agent/themes/*.json`
- Project: `.athena/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `athena.themes` entries in `package.json`
...
On first run, athena detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.athena/agent/themes
vim ~/.athena/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/athena-agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, athena reloads it automatically for immediate visual feedback.
...
Athena uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, athena falls back to the nearest approximation.
