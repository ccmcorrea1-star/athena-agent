> athena can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@athena/tui`](https://github.com/earendil-works/athena-agent/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@athena/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `ATHENA_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@athena/tui";
...
Import from `@athena/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@athena/tui";
```
...
```typescript
import { matchesKey, Key } from "@athena/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@athena/tui";
...
```typescript
import { getMarkdownTheme } from "@athena/coding-agent";
import { Markdown } from "@athena/tui";
...
Set `ATHENA_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
ATHENA_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@athena/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@athena/coding-agent";
import { DynamicBorder } from "@athena/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@athena/tui";
...
```typescript
import { BorderedLoader } from "@athena/coding-agent";

athena.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@athena/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@athena/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@athena/tui";

athena.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while athena is streaming a response.
...
// Restore athena's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@athena/tui";
