...
export class Connection {
	...
	constructor(options: ConnectionOptions) {
		...
		if (
			...
			throw new TypeError(`AthenaClient maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
		}
	}
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#lifecycle.state !== "disconnected") {
			return Promise.reject(new AthenaDisconnectedError(`AthenaClient is already ${this.#lifecycle.state}`));
		}
	...
	disconnect(reason: string | Error = "Client disconnected"): void {
		...
		this.#failAndClose(typeof reason === "string" ? new AthenaDisconnectedError(reason) : reason);
	}
	...
	send(frame: Uint8Array): void {
		...
		if (lifecycle.state !== "connected") throw new AthenaDisconnectedError();
	...
	#handleMessage(message: ServerMessage): void {
		...
		if (lifecycle.state === "connecting") {
			if (message.type === "hello_error") {
				this.#failAndClose(new AthenaServerError(message.error));
				return;
			}
	...
	#handleClose(): void {
		...
		let error: Error = new AthenaDisconnectedError("Byte transport closed");
		try {
