...
import { cloudflareAIGatewayProvider } from "@athena/ai/providers/cloudflare-ai-gateway";
...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".athena/skills")),
		source("user", join(homedir(), ".athena/agent/skills")),
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".athena/prompts")), source("user", join(homedir(), ".athena/agent/prompts"))],
