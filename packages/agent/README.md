...
Stateful agent with tool execution and event streaming. Built on `@athena/ai`.

## Installation

```bash
npm install @athena/agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@athena/storage-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other storage backends to ship as their own packages in the future.

## Quick Start

```typescript
import { Agent } from "@athena/agent-core";
...
```typescript
declare module "@athena/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@athena/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@athena/agent-core";
