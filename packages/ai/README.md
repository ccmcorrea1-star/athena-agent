...
TypeBox exports are re-exported from `@athena/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@athena/ai';
...
```typescript
import { anthropicProvider } from '@athena/ai/providers/anthropic';
import { openaiProvider } from '@athena/ai/providers/openai';
import { openrouterProvider } from '@athena/ai/providers/openrouter';
import { amazonBedrockProvider } from '@athena/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@athena/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@athena/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@athena/ai';
...
```typescript
import { validateToolCall, type Tool } from '@athena/ai';
...
```typescript
import { builtinImagesModels } from '@athena/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@athena/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@athena/ai';
import { openAICompletionsApi } from '@athena/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@athena/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@athena/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@athena/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@athena/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@athena/ai';
import { anthropicProvider } from '@athena/ai/providers/anthropic';
import { openaiProvider } from '@athena/ai/providers/openai';
import { googleProvider } from '@athena/ai/providers/google';
...
- `@athena/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@athena/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@athena/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@athena/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@athena/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@athena/ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @athena/ai login              # interactive provider selection
npx @athena/ai login anthropic    # login to specific provider
npx @athena/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@athena/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@athena/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@athena/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@athena/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@athena/ai`.
