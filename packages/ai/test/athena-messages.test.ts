...
import { type AthenaMessagesOptions, stream, streamSimple } from "../src/api/athena-messages.ts";
...
describe("pi-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		const options: AthenaMessagesOptions = {
			apiKey: "test-key",
			debug: true,
			onResponse: (response) => {
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("athena_messages_response_failure");
