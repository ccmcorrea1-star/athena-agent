 ...
 *
 * NOTE: The wire protocol identifier is `"pi-messages"`, not `"athena-messages"`.
 * This name is historical and MUST NOT be changed — it is used by the Radius
 * gateway, user-facing models.json configurations, and external integrations.
 */
...
/** Serialized assistant-message event as sent by an athena-messages backend. */
...
function createErrorEvent(model: Model<"pi-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof AthenaMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("athena_messages_response_failure", error, error.diagnosticDetails),
		);
	}
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	return getProviderEnvValue("ATHENA_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"pi-messages", AthenaMessagesOptions> = (
	model: Model<"pi-messages">,
	context: Context,
	...
	void (async () => {
		try {
			...
			for await (const piEvent of readAthenaMessagesEvents(response.body)) {
