...
import { type AthenaCodingAgentInput, createAthenaCodingAgentHarness } from "./athena-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasAthenaDocs: boolean;
	...
	return createAthenaCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasAthenaDocs: session.systemPrompt.includes("\nAthena documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Athena system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Athena system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<AthenaCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@athena/coding-agent")) {
				failures.push("extension does not import the canonical @athena/coding-agent package");
			}
...
const extensionHarnessTable = evalHarnessTable("Athena extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Athena extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create an Athena extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Athena eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasAthenaDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
