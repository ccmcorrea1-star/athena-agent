...
import { ATHENA_SESSION_SNAPSHOT_ARTIFACT } from "./vitest-evals/artifacts.ts";

export type AthenaCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type AthenaCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type AthenaCodingAgentHarnessOptions = {
	name?: string;
	model?: AthenaCodingAgentModelSelection;
	noTools?: CreateAgentSessionOptions["noTools"];
	transformSystemPrompt?: (defaultPrompt: string) => string;
};

type AthenaCodingAgentHarnessWithOutput<TOutput extends JsonValue> = AthenaCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: AthenaCodingAgentModelSelection | undefined,
	environment: { ATHENA_PROVIDER?: string; ATHENA_MODEL?: string } = process.env,
): AthenaCodingAgentModelSelection {
	const provider = (explicitModel?.provider ?? environment.ATHENA_PROVIDER)?.trim();
	const id = (explicitModel?.id ?? environment.ATHENA_MODEL)?.trim();
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both ATHENA_PROVIDER and ATHENA_MODEL as defaults.");
	}
	return { provider, id };
}
...
async function runAthenaCodingAgent<TOutput extends JsonValue>(
	input: AthenaCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: AthenaCodingAgentHarnessOptions | AthenaCodingAgentHarnessWithOutput<TOutput>,
	...
	const root = await mkdtemp(join(tmpdir(), "athena-eval-"));
	...
	try {
		...
		try {
			...
			if (response === undefined) throw new Error("Athena eval input must include at least one prompt step.");
	...
	if (sessionManager) {
		try {
			...
			if (sessionPath && existsSync(sessionPath)) {
				setArtifact(ATHENA_SESSION_SNAPSHOT_ARTIFACT, await readFile(sessionPath, "utf8"));
			}
		} catch (error) {
...
export function createAthenaCodingAgentHarness<TOutput extends JsonValue>(
	options: AthenaCodingAgentHarnessWithOutput<TOutput>,
): Harness<AthenaCodingAgentInput, TOutput>;
export function createAthenaCodingAgentHarness(
	options?: AthenaCodingAgentHarnessOptions,
): Harness<AthenaCodingAgentInput, string>;
export function createAthenaCodingAgentHarness<TOutput extends JsonValue>(
	options: AthenaCodingAgentHarnessOptions | AthenaCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<AthenaCodingAgentInput, string | TOutput>({
		name: options.name ?? "athena-coding-agent",
		run: ({ input, signal, setArtifact }) => runAthenaCodingAgent(input, signal, setArtifact, options),
	});
}
