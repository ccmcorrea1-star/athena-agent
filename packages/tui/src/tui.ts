...
export const VIEWPORT_TUI = Symbol.for("@athena/tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private clearOnShrink = process.env.ATHENA_CLEAR_ON_SHRINK === "1";
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ?? process.env.ATHENA_CODING_AGENT_DIR ?? path.join(os.homedir(), ".athena", "agent");
