...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			private: true,
			dependencies: {
				"@athena/coding-agent": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@athena/ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@athena/coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@athena/ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@athena/coding-agent"], "^1.0.0");
