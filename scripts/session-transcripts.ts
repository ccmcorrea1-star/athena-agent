 ...
 *   --analyze      Spawn athena subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("athena", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn athena: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".athena/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn athena subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".athena/agent/AGENTS.md");
